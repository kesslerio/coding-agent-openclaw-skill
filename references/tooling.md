...
The wrapper scripts run Codex inside tmux and emit monitoring info:
...
`scripts/tmux-run` standardizes sockets, logging, and session names:

```bash
# Run an implementation command in tmux
CODEX_TMUX_SESSION_PREFIX=codex-impl \
  ./scripts/tmux-run timeout 180s codex --yolo exec "Implement feature X"

# Run a review in tmux
CODEX_TMUX_SESSION_PREFIX=codex-review \
  ./scripts/tmux-run timeout 300s codex review --base main --title "PR Review"
```
